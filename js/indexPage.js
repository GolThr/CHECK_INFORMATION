...
function init(){
    ...
    console.log(bak_tbl_data);
    console.log(bak_tbl_row);
    console.log(bak_tbl_col);
    if(bak_tbl_data != undefined){
        showDialogTip('温馨提示', '您尚有未保存的表单，要继续编辑吗？');
        $('.dialog_btn_main').on('click', function (){
...
function renderTable(){
    ...
    for(var i = 0; i < TBL_ROW; i++){
        ...
        for(var j = 0; j < TBL_COL; j++){
            ...
            if(i == 0){
                $('#row'+i).append('<td class="col'+ j +'" col="'+ j +'" onclick="onColSelected(this)">' + data + '</td>');
            }else if(j == 0){
